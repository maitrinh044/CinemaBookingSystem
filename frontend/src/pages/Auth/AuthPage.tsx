...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (!regName.trim() || !regEmail.trim() || !regPhone.trim() || !regPassword.trim()) {
      const msg = 'Vui lòng điền đầy đủ các trường thông tin.';
      setErrorMsg(msg);
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Card Container */}
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-[var(--border-color)] shadow-2xl space-y-5 text-left">
          {/* Logo & Header */}
          <div className="text-center space-y-1.5 pb-2">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-[var(--logo-icon)] text-white shadow-lg shadow-red-600/30 mx-auto">
              <Clapperboard className="w-6 h-6" />
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-[var(--logo-text)]">
              CineGlow
            </h1>
            <p className="text-xs text-[var(--text-sub)]">
              {tab === 'login'
                ? 'Đăng nhập để nhận ưu đãi vé VIP & tích điểm CinePoint'
                : tab === 'register'
                ? 'Đăng ký thành viên CineGlow nhận ngay 50 điểm thưởng'
                : 'Khôi phục mật khẩu tài khoản CineGlow'}
            </p>
          </div>
          ...
          {tab === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="page-login-remember" className="text-xs text-[var(--text-sub)] cursor-pointer select-none">
                  Ghi nhớ đăng nhập
                </label>
              </div>
              ...
              {/* Switch to Register */}
              <div className="text-center text-xs text-[var(--text-sub)] pt-2">
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegisterSubmit} className="space-y-3.5">
              ...
              <div className="flex items-start gap-2 pt-1">
                ...
                />
                <label htmlFor="page-reg-agree" className="text-[11px] text-[var(--text-sub)] cursor-pointer leading-tight select-none">
                  Tôi đồng ý với <span className="text-[var(--primary)] font-semibold">Điều khoản sử dụng</span> của CineGlow.
                </label>
              </div>
            ...
            <form onSubmit={handleForgotSubmit} className="space-y-4">
              {forgotSent ? (
                <div className="text-center space-y-3 py-3">
                  ...
                  <p className="text-xs text-[var(--text-sub)]">
                    Vui lòng kiểm tra hộp thư đến của <b>{forgotEmail}</b> để nhận liên kết đặt lại mật khẩu.
                  </p>
                  <Button
                    type="button"
                    variant="primary"
